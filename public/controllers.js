(() => {
  ...
  function ensureDom() {
    ...
    dom.finalScreen.root = byId("final-summary-screen");
    dom.finalScreen.veil = dom.finalScreen.root ? dom.finalScreen.root.querySelector("[data-final-overlay-close='true']") : null;
    dom.finalScreen.title = byId("final-summary-screen-title");
    dom.finalScreen.subtitle = byId("final-summary-screen-subtitle");
    dom.finalScreen.narrative = byId("final-summary-screen-narrative");
    dom.finalScreen.body = byId("final-summary-screen-body");
    dom.finalScreen.restart = byId("final-summary-screen-restart");
    dom.finalScreen.close = byId("final-summary-screen-close");
  }
  ...
  let pendingFinalSummaryCredits = false;

  function triggerFinalCreditsIfPending() {
    if (!pendingFinalSummaryCredits) return;
    pendingFinalSummaryCredits = false;
    setTimeout(() => {
      showCreditsScreen();
    }, 160);
  }

  function lockBodyForFinalScreen(enable) {
    if (typeof document === "undefined" || !document.body) return;
    document.body.classList.toggle("final-screen-open", Boolean(enable));
  }

  function updateFinalScreenNarrative() {
    const text = state.finalNarrative ? String(state.finalNarrative) : "";
    if (!dom.finalScreen.narrative) return;
    if (!text.trim()) {
      setText(dom.finalScreen.narrative, "");
      setHidden(dom.finalScreen.narrative, true);
      return;
    }
    setText(dom.finalScreen.narrative, text);
    setHidden(dom.finalScreen.narrative, false);
  }

  function renderFinalScreenContent(html) {
    if (dom.finalScreen.body) {
      dom.finalScreen.body.innerHTML = String(html == null ? "" : html);
    }
    updateFinalScreenNarrative();
  }

  function finalScreenIsOpen() {
    ensureDom();
    return !!(dom.finalScreen.root && !dom.finalScreen.root.hasAttribute("hidden"));
  }

  function showFinalScreen(html) {
    ensureDom();
    overlayClose();
    if (!dom.finalScreen.root) return;
    renderFinalScreenContent(html);
    setHidden(dom.finalScreen.root, false);
    lockBodyForFinalScreen(true);
  }

  function hideFinalScreen(triggerCredits) {
    ensureDom();
    if (!dom.finalScreen.root) return;
    const wasOpen = finalScreenIsOpen();
    setHidden(dom.finalScreen.root, true);
    lockBodyForFinalScreen(false);
    if (wasOpen && triggerCredits) {
      triggerFinalCreditsIfPending();
    }
  }

  function restartGameFromFinalScreen() {
    const restart = typeof window !== "undefined" ? window.__restartGame : null;
    const attempt = () => {
  ...
  function setupEventsOnce() {
    ...
    bindOnce(dom.overlay.scrim, "overlay_scrim", "click", () => {
      console.log('[DEBUG] SCRIM CLICK', state.overlay.modal, state.phone.status);
      
      // ✅ CENTRALIZA recusa de tentação no handler único
      if (window.tentacaoAtiva) {
        console.log('[DEBUG] SCRIM TENTACAO → recusarTentacao');
        recusarTentacao(window.tentacaoAtiva);
        return;
      }
      
      if (state.overlay.modal === "phone" && state.phone.status !== "idle") {
        phoneHang();
        return;
      }
    ...
    bindOnce(dom.finalScreen.veil, "final_veil", "click", () => {
      hideFinalScreen(true);
    });

    bindOnce(dom.finalScreen.close, "final_close", "click", () => {
      hideFinalScreen(true);
    });

    bindOnce(dom.finalScreen.restart, "final_restart", "click", () => {
      pendingFinalSummaryCredits = false;
      hideFinalScreen(false);
      restartGameFromFinalScreen();
    });
  ...
  function showFinalSummaryModal() {
    ...
    pendingFinalSummaryCredits = true;
    showFinalScreen(buildFinalComparisonHtml(currentSnapshot, previousSession));
  ...
  function setFinalNarrative(value) {
    state.finalNarrative = String(value == null ? "" : value);
    return state.finalNarrative;
  }
