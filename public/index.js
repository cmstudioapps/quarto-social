window.addEventListener("DOMContentLoaded", async () => {
  ...
  function askDecisionConfirmation(cost) {
    return new Promise((resolve) => {
      ...
      controllers.abrirModal({
        ...
        text: `deseja gastar ${cost} reais para esse problema?`,
        buttons: [
          {
            text: 'Sim, gastar',
            tone: 'green',
  ...
  async function handleDecision(option, legend) {
    ...
    if (hasCost) {
      ...
      const confirmed = await askDecisionConfirmation(cost);
      if (!confirmed) {
        controllers.notificar(`Decisão cancelada: "${selectionText}".`, { type: 'info' });
        return;
      }
      controllers.addDinheiro(-cost);
      controllers.notificar(`Gasto confirmado: -R$ ${cost}.`, { type: 'success' });
    }
