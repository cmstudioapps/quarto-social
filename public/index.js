...
function aplicarImpactosTemptacao(efeitos) {
  ...
  function recusarTentacao(tentacao) {
    if (!tentacao || !tentacao.reject || !tentacao.resolve) return false;
    
    console.log('[DEBUG] recusarTentacao executando');
    
    // 1. Aplica efeitos da rejeição
    const scaledReject = aplicarImpactosTemptacao(tentacao.reject.effects);
    
    // 2. Resolve promise (libera mostrarTentacao)
    tentacao.resolve('rejected');
    
    // 3. Feedback prefeito (async, sem await)
    if (tentacao.reject.prefeitoMessage) {
      controllers.ligacao.setLegendaPrefeito(tentacao.reject.prefeitoMessage, 2400).catch(console.error);
    }
    
    // 4. Notificação
    const notificationText = tentacao.reject.notification || 'Você recusou a tentação.';
    controllers.notificar(notificationText, { type: 'success' });
    
    // 5. Cleanup completo
    tentacaoAtiva = null;
    window.tentacaoAtiva = null;
    lockTentacaoButtons(false);
    controllers.ligacao.mostrarBotaoChamada(false);
    
    console.log('[DEBUG] recusarTentacao concluída');
    return true;
  }
...
controllers.ligacao.aoDesligar(({ name }) => {
    console.log('[DEBUG] aoDesligar → recusarTentacao');
    if (!tentacaoAtiva || tentacaoAtiva.person !== name) return;
    recusarTentacao(tentacaoAtiva);
    controllers.ligacao.desligar();
  });

// ✅ CORREÇÃO BUG: Qualquer fechamento (X, ESC, scrim) durante tentação = recusa + resolve
controllers.ligacao.aoFechar(({ name, status }) => {
    console.log('[DEBUG] aoFechar → recusarTentacao');
    if (tentacaoAtiva && tentacaoAtiva.person === name) {
      recusarTentacao(tentacaoAtiva);
    }
  });

async function mostrarTentacao(tentacao) {
    console.log('[DEBUG] mostrarTentacao START', tentacao.id);
    if (!tentacao) return;
    const decisionPromise = new Promise((resolve) => {
      window.tentacaoAtiva = { ...tentacao, resolve };  // Global pra scrim interceptar
      tentacaoAtiva = window.tentacaoAtiva;
    });
    ...
    console.log('[DEBUG] mostrarTentacao await decisionPromise START');
    const result = await Promise.race([decisionPromise, fallbackPromise]);
    console.log('[DEBUG] mostrarTentacao END result:', result);
  ...
  async function handleDecision(option, legend) {
    ...
    if (hasCost) {
      ...
      forcedNegative = cost > currentMoney && !affordableOptionExists;
    ...
    if (forcedNegative) {
      await finalizarMandatoPorFaltaDeDinheiro();
    }
  }
  ...
  const FINAL_ROUND_MARKER = 6;

  async function mostrarFinal() {
    controllers.setRoundAtual(FINAL_ROUND_MARKER);
    ...
    controllers.setRoundAtual(FINAL_ROUND_MARKER + 1);
  }

  async function finalizarMandatoPorFaltaDeDinheiro() {
    if (sessionTerminated) return;
    sessionTerminated = true;
    await alert('O dinheiro acabou. Seu mandato será encerrado imediatamente para você conferir as métricas e os créditos finais.');
    controllers.setRoundAtual(FINAL_ROUND_MARKER + 1);
  }
  ...
  async function runSession() {
    ...
    try {
      ...
      finalNarrative = `${nome}, sua gestão chegou ao fim. Confira a tela final e reflita sobre suas decisões.`;
      controllers.setFinalNarrative(finalNarrative);

      controllers.definirRounds(1, FINAL_ROUND_MARKER);
